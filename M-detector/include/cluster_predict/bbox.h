...
public:
    ...
    void Box_clip(const bbox_s &box_src, const bbox_s &box_tar, std::vector<Plane> &plane_clipped)
    {
        ...
        // std::cout << "vertices_tar_src: " << vertices_tar_src << std::endl;
        ...
        {   
            ...
            for (int j = 0; j < coincide_tags.size(); j++)
            {
                ...
                {
                    // std::cout << "coincide happen: " << coincide_tags[j].transpose() << std::endl;
                    continue_tag = true;
                    break;
                }
            }
            ...
            coincide_tag[1] = i;
            // std::cout << "The " << i << " face begin to clip " << std::endl;
            // std::cout << "It origin contains: " << std::endl;
            // for (int j = 0; j < planes[i].size(); j++)
            // {
            //     std::cout << planes[i][j].transpose() << std::endl;
            // }
            Poly_clip_by_box(planes[i], box_src);
            if(planes[i].size() > 2)
            {   
                // std::cout << "At last It is clipped to: " << std::endl;
                for (int j = 0; j < planes[i].size(); j++)
                {   
    ...
    double Convexhull_volume(const std::vector<Plane> &plane_clipped)
    {
        ...
        int vertice_cout = 0;
        // std::cout << "plane size after clipping: " << plane_clipped.size() << std::endl;
        for (int i = 0; i < plane_clipped.size(); i++)
        {
            for (int j = 0; j < plane_clipped[i].size(); j++)
            {
                vertice_cout ++;
                // std::cout << "plane " << i << " contains " << plane_clipped[i][j].transpose() << std::endl;
        ...
        {   
            ...
            // std::cout << "area: " << area << " height: " << height << std::endl;
        }
        return volume_total;
    }
