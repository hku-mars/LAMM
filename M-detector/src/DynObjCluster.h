...
// #define Grid_size_1d    3
// #define Grid_size       27  //Grid_size_1d ^ 3
#define HASH_length     10000
// #define Voxel_revolusion   0.3
...
struct bbox_t
{
    ...
    // std::vector<Eigen::Vector3f>                                    Ground_norm;
    ...
    void reset()
    {
      ...
      OOBB_R.clear();
      // std::vector<std::vector<int>>().swap(Point_indices);
      // std::vector<pcl::PointCloud<PointType>>().swap(Point_cloud);
      // std::vector<geometry_msgs::PoseWithCovarianceStamped>().swap(Center);
      // std::vector<pcl::PointCloud<PointType>>().swap(Ground_points);
      // std::vector<pcl::PointCloud<PointType>>().swap(true_ground);
      // std::vector<std::unordered_set<VOXEL>>().swap(Ground_voxels_set);
      // std::vector<std::vector<VOXEL>>().swap(Ground_voxels_vec);
      // std::vector<Eigen::Vector3f>().swap(OOBB_Min_point);
      // std::vector<Eigen::Vector3f>().swap(OOBB_Max_point);
      // std::vector<Eigen::Matrix3f>().swap(OOBB_R);
    }
};
